...
# Set variables
CURRENT_PLATFORM=""
# ROMS_ROOT="games-internal/roms"  # lakka local storage path
ROMS_ROOT="/storage/emulated/0/Download/roms"  # Android Download ROMs folder
lakka_HOST="192.168.0.132"
lakka_USER="root"
lakka_PASS="root"  # lakka password
lakka_ROMS_PATH="/storage/roms"  # lakka storage path
...
get_platform_dir() {
    local platform="$1"
    local base_dir="$ROMS_ROOT"  # Android Download ROMs folder
...
detect_and_connect_smb() {
    ...
    case $connection_choice in
        1|2|3|4) # Scan network (MacOS, Windows, Linux, or Android)
            ...
            echo "Checking if $lakka_HOST is running SMB..."
            if nc -z -G 1 "$lakka_HOST" 445 2>/dev/null || nc -z -G 1 "$lakka_HOST" 139 2>/dev/null; then
                echo "✅ SMB service detected on $lakka_HOST"
            else
                echo "⚠️ No SMB service detected on $lakka_HOST"
            fi
            ...
            if [ ${#smb_hosts[@]} -eq 0 ]; then
                ...
                echo "Adding $lakka_HOST as a fallback option."
                smb_hosts+=("$lakka_HOST (Known SMB Server)")
            fi
    ...
    if [ ${#smb_shares[@]} -eq 0 ]; then
        echo "No SMB servers found on the network."
        echo "Adding $lakka_HOST as a fallback option."
        smb_shares+=("$lakka_HOST (Known SMB Server)")
    fi
    ...
    if [ "$os_type" == "MacOS" ] || [ -z "$os_type" ]; then
        ...
        if [ -n "$lakka_USER" ] && [ -n "$lakka_PASS" ]; then
            mount_cmd="mount -t smbfs //$lakka_USER:$lakka_PASS@$selected_host/$clean_share $mount_point$local_connection_options"
        else
        ...
        if [ -n "$lakka_USER" ] && [ -n "$lakka_PASS" ]; then
            mount_cmd="mount -t cifs //$selected_host/$clean_share $mount_point -o username=$lakka_USER,password=$lakka_PASS$local_connection_options"
        else
...
download_rom() {
    ...
    if [ $download_status -eq 0 ]; then
        ...
        # Transfer to lakka using sshpass
        echo "Transferring to lakka..."
        if sshpass -p "$lakka_PASS" ssh -o StrictHostKeyChecking=no "$lakka_USER@$lakka_HOST" "mkdir -p /storage/roms/$platform" && \
           sshpass -p "$lakka_PASS" scp -o StrictHostKeyChecking=no "$platform_dir/$decoded_rom_name" "$lakka_USER@$lakka_HOST:/storage/roms/$platform/"; then
            echo "ROM transferred to lakka successfully"
            echo "Lakka"
            echo "ROM available at: /storage/roms/$platform/$decoded_rom_name"
        else
            echo "Failed to transfer ROM to lakka"
            return 1
        fi
    else
