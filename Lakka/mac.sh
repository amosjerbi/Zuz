...
ROMS_ROOT="$HOME/Desktop/Aroms"  # Mac Desktop ROMs folder
ROCKNIX_HOST="192.168.0.132"
ROCKNIX_USER="root"
ROCKNIX_PASS="root"  # RockNix password
...
menu_select() {
    ...
    draw_menu() {
        # Clear screen from cursor down and move to start
        tput el
        tput ed
...
download_rom() {
    ...
    if [ $download_status -eq 0 ]; then
        ...
        if sshpass -p "$ROCKNIX_PASS" ssh -o StrictHostKeyChecking=no "$ROCKNIX_USER@$ROCKNIX_HOST" "mkdir -p /storage/roms/$platform" && \
           sshpass -p "$ROCKNIX_PASS" scp -o StrictHostKeyChecking=no "$platform_dir/$decoded_rom_name" "$ROCKNIX_USER@$ROCKNIX_HOST:/storage/roms/$platform/"; then
