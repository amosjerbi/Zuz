...
scan_for_smb_hosts() {
    ...
    # Use background processes for faster scanning
    pids=()
    ...
    # Collect results
    if [ -f /tmp/smb_hosts.txt ]; then
        # Sort and remove duplicates
        sort -u /tmp/smb_hosts.txt > /tmp/smb_hosts_sorted.txt
        
        # Read into array
        while read -r host; do
            DETECTED_SMB_HOSTS+=("$host")
        done < /tmp/smb_hosts_sorted.txt
        
        # Clean up temp files
        rm -f /tmp/smb_hosts.txt /tmp/smb_hosts_sorted.txt
    fi
