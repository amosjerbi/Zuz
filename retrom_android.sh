...
PS1_DIR="$ROMS_ROOT/ps1"
PS2_DIR="$ROMS_ROOT/ps2"
N64_DIR="$ROMS_ROOT/n64"
DREAMCAST_DIR="$ROMS_ROOT/dreamcast"
...
detect_and_connect_smb() {
    ...
    if [ ${#smb_shares[@]} -eq 0 ]; then
        echo "No SMB servers found on the network."
        echo "Adding 192.168.0.132 as a fallback option."
        smb_shares+=("192.168.0.132 (Known SMB Server)")
    fi
    ...
    if [ "$os_type" == "MacOS" ] || [ -z "$os_type" ]; then
        ...
        if [ -n "$username" ] && [ -n "$password" ]; then
            mount_cmd="mount -t smbfs //$username:$password@$selected_host/$clean_share $mount_point$local_connection_options"
        else
        ...
        if [ -n "$username" ] && [ -n "$password" ]; then
            mount_cmd="mount -t cifs //$selected_host/$clean_share $mount_point -o username=$username,password=$password$local_connection_options"
        else
...
download_rom() {
    ...
    if [ $download_status -eq 0 ]; then
        ...
        transfer_to_rocknix "$platform_dir/$decoded_rom_name" "$platform"
    else
...
setup_ssh_key() {
    # Check if we already have SSH key access
    ssh -o BatchMode=yes -o StrictHostKeyChecking=no ${ROCKNIX_USER}@${ROCKNIX_HOST} "exit" 2>/dev/null
    if [ $? -eq 0 ]; then
        return 0
    fi
    ...
    cat > "$temp_script" << EOF
#!/usr/bin/expect -f
spawn ssh-copy-id -o StrictHostKeyChecking=no ${ROCKNIX_USER}@${ROCKNIX_HOST}
expect "password:"
send "${ROCKNIX_PASS}\r"
expect eof
EOF
...
# Function to transfer ROM to RockNix
transfer_to_rocknix() {
    local rom_path="$1"
    local platform="$2"
    
    if [ -f "$rom_path" ]; then
        # Copy ROM to Android Download directory first
        local android_download_dir="/storage/emulated/0/Download/roms/$platform"
        mkdir -p "$android_download_dir"
        cp "$rom_path" "$android_download_dir/"
        
        local rom_filename=$(basename "$rom_path")
        local android_rom_path="$android_download_dir/$rom_filename"
        
        # Transfer ROM to RockNix using sshpass
        echo "Transferring ROM to RockNix..."
        if sshpass -p "${ROCKNIX_PASS}" ssh -o StrictHostKeyChecking=no "${ROCKNIX_USER}@${ROCKNIX_HOST}" "mkdir -p ${ROCKNIX_ROMS_PATH}/${platform}" && \
           sshpass -p "${ROCKNIX_PASS}" scp -o StrictHostKeyChecking=no "$android_rom_path" "${ROCKNIX_USER}@${ROCKNIX_HOST}:${ROCKNIX_ROMS_PATH}/${platform}/"; then
...
verify_rom_directories() {
    ...
    if [ ! -d "$ROMS_ROOT" ]; then
        echo "The ROM directory $ROMS_ROOT does not exist."
        all_ok=false
    fi
...
copy_roms_to_external() {
    ...
    for platform in "nes" "snes" "genesis" "gb" "gba" "gg" "ngp" "sms"; do
        local src_dir="$ROMS_ROOT/$platform"
