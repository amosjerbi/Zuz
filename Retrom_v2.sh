...
search_roms() {
    ...
    echo "Use arrow keys to navigate, Enter to select a ROM to download, ESC to cancel"
    echo "-----------------------------------------------------"
    
    # Display ROMs with arrow key selection
    menu_select "${filtered_roms[@]}"
    local selected=$?
    
    # Check if user pressed ESC (selected will be 0)
    if [ $selected -eq 0 ]; then
        echo "ROM selection cancelled."
        return 0
    fi
    
    # Download the selected ROM
    local selected_rom="${filtered_roms[$((selected-1))]}"
    echo "You selected: $selected_rom"
    
    # Ask for confirmation
    echo "Download this ROM?"
