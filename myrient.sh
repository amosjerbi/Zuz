...
get_archive_url() {
    local platform="$1"
    local base_url="https://myrient.erista.me/files/No-Intro"
...
menu_select() {
   ...
   # Function to draw menu
    draw_menu() {
        ...
        # Print header to emphasize number selection
        echo "============================================="
        echo "SELECT BY NUMBER: Type a number and press Enter"
        echo "- OR USE: Arrow keys (↑/↓) and Enter to select"
        echo "============================================="
        echo ""
    ...
    # Handle direct number input first
    echo ""
    echo "Enter number (1-$num_options): "
    # Create a variable to store direct number input
    read -t 1 -n 1 direct_num
    
    # If user entered a number directly, process it
    if [[ $direct_num =~ [0-9] ]]; then
        # Start collecting digits for multi-digit numbers
        local number=$direct_num
        echo -n "$direct_num"
        
        # Keep reading digits until Enter is pressed
        while read -t 2 -n 1 next_digit; do
            if [[ $next_digit =~ [0-9] ]]; then
                number="${number}${next_digit}"
                echo -n "$next_digit"
            elif [[ $next_digit == "" ]]; then
                # Enter key pressed
                break
            fi
        done
    ...
    while true; do
        ...
        if [[ $key =~ [0-9] ]]; then
            ...
            local number=$key
            echo -n "$key"
            local timeout=1  # Whole number timeout for compatibility
            
            # Set up a timeout for reading additional digits
            while read -rsn1 -t $timeout next_key; do
...
search_roms() {
    local search_term="$1"
    local platform="${2:-$CURRENT_PLATFORM}"
    
    # Use a directory that's likely to be accessible on Android
    local temp_dir="$HOME"
    if [ -d "/storage/emulated/0/Download" ]; then
        # If we're on Android, use the Download directory
    ...
    if [ -z "$archive_url" ]; then
        echo "Error: Archive URL not found for platform $platform"
        return 1
    fi
    ...
    if [ ! -w "$rom_list_file" ]; then
        echo "❌ Error: Cannot write to $rom_list_file"
        ...
        if [ ! -w "$rom_list_file" ]; then
            echo "❌ Error: Cannot write to $rom_list_file either"
            echo "Please run the script from a directory where you have write permissions."
            return 1
        fi
    fi

    echo "Searching for ROMs on $archive_url..."
    
    if [ "$platform" = "gamegear" ]; then
        # Extract ROM filenames from the HTML table structure
        curl -s "$archive_url" | grep -o '<tr>.*<td class="link"><a href="[^"]*" title="[^"]*">[^<]*</a>' | sed 's/.*title="\([^"]*\)">.*/\1/' > "$rom_list_file" 2>/dev/null
    else
        # ... existing code for other platforms ...
        curl -s "$archive_url" | grep -o 'href="[^"]*"' | sed 's/href="//;s/"$//' > "$rom_list_file" 2>/dev/null
    fi

    if [ ! -s "$rom_list_file" ]; then
        echo "No ROMs found at $archive_url"
        # Remove the temporary file if it exists
        rm -f "$rom_list_file" 2>/dev/null
        return 1
    fi
    ...
    while IFS= read -r rom; do
        ...
        # Convert HTML entities and URL decode the ROM name
        decoded_name=$(echo "$rom" | sed 's/&amp;/\&/g' | sed 's/%\([0-9A-F][0-9A-F]\)/\\x\1/g' | xargs -0 echo -e)

        # If search term is empty or ROM name contains the search term (case insensitive)
        if [ -z "$search_term" ] || echo "$decoded_name" | grep -qi "$search_term"; then
            filtered_roms+=("$decoded_name")
            ((rom_count++))
        fi
    done < "$rom_list_file"
    ...
    # Store the filtered ROMs in a global array so we can return to this list after download
    CURRENT_SEARCH_RESULTS=("${filtered_roms[@]}")
    CURRENT_SEARCH_TERM="$search_term"
    
    # Loop to allow multiple downloads from the same search results
    while true; do
        clear
        echo "Found $rom_count ROMs matching '$search_term':"
        
        # Display options for the user with numbers
        local display_options=("Download ALL matching ROMs" "${filtered_roms[@]}" "Return to Main Menu")
        ...
        # Check if user selected the first option (Download All ROMs)
            ...
            echo "Downloading all matching ROMs..."
            download_all_matching_roms "${filtered_roms[@]}" "$platform"
            # After downloading all, return to main menu
            break
        # User selected a specific ROM
        elif [ $selected -gt 1 ] && [ $selected -lt ${#display_options[@]} ]; then
            # Adjust index to account for "Download ALL" option
            local rom_index=$((selected-2))
            download_single_rom "${filtered_roms[$rom_index]}" "$platform"
            
            # Don't break here - loop back to show search results again
            echo "Returning to search results..."
            sleep 1
        fi
    done
...
# Function to download a single ROM
download_single_rom() {
    local rom_file="$1"
    local platform="$2"
    ...
    # Wait for user to acknowledge before returning to search results
    read -p "Press Enter to return to search results..."
    
    # No return statement here - let the function caller handle returning to search results
}

# Function to download all matching ROMs
download_all_matching_roms() {
    local platform="${@: -1}" # Last argument is the platform
    local roms=("${@:1:$#-1}") # All arguments except the last one are ROMs
    local total_roms=${#roms[@]}
    local success_count=0
    local fail_count=0
    
    echo "Downloading $total_roms ROMs for platform: $(to_uppercase $platform)"
...
download_all_roms() {
    ...
    echo "Downloading all ROMs for platform: $(to_uppercase $platform)"
    ...
    echo "Fetching list of ROMs from $archive_url..."
    ...
    local total_roms=$(grep -c . "$rom_list_file")
    ...
    echo "Found $total_roms ROMs. Downloading all..."
    
    # Confirm download
    echo "WARNING: This will download $total_roms ROMs. Continue? (y/n)"
    read -p "> " confirm
    ...
    while IFS= read -r rom_file; do
        ...
        echo "[$counter/$total_roms] Downloading: $decoded_rom_name"
