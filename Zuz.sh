...
scan_for_smb_hosts() {
    ...
    # Always check the known RockNix host from user's configuration
    known_rocknix_host="192.168.0.132"
    ...
    # First priority: Try direct SSH connection to the known RockNix host
    echo "Attempting direct SSH connection to known RockNix host: $known_rocknix_host..."
    
    # Always check the specific IP 192.168.0.159 directly
    specific_ip="192.168.0.159"
    echo "Checking specific IP: $specific_ip..."
    if ping -c 1 -W 2 "$specific_ip" >/dev/null 2>&1; then
        echo "✅ $specific_ip is reachable via ping"
        
        # Check if SSH port is open
        if nc -z -w 2 "$specific_ip" 22 >/dev/null 2>&1; then
            echo "✅ SSH service detected on $specific_ip"
            DETECTED_SMB_HOSTS+=("$specific_ip")
            echo "Added $specific_ip to detected hosts"
        elif nc -z -w 2 "$specific_ip" 445 >/dev/null 2>&1; then
            echo "✅ $specific_ip has SMB service (port 445 open)"
            DETECTED_SMB_HOSTS+=("$specific_ip")
            echo "Added $specific_ip to detected hosts"
        elif nc -z -w 2 "$specific_ip" 139 >/dev/null 2>&1; then
            echo "✅ $specific_ip has SMB service (port 139 open)"
            DETECTED_SMB_HOSTS+=("$specific_ip")
            echo "Added $specific_ip to detected hosts"
        else
            echo "❌ $specific_ip has no SMB/SSH service detected"
        fi
    else
        echo "❌ $specific_ip is not reachable via ping"
    fi
    
    # Check if sshpass is available
    if command -v sshpass >/dev/null 2>&1; then
        echo "Using sshpass for authentication..."
        if sshpass -p "$ROCKNIX_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 "$ROCKNIX_USER@$known_rocknix_host" "echo Connection successful" >/dev/null 2>&1; then
            echo "✅ Direct SSH connection to $known_rocknix_host successful!"
            DETECTED_SMB_HOSTS+=("$known_rocknix_host")
            ROCKNIX_HOST="$known_rocknix_host"
            
            # Since we've confirmed SSH works, try to verify the RockNix ROMs path exists
            echo "Verifying RockNix ROMs path..."
            if sshpass -p "$ROCKNIX_PASS" ssh -o StrictHostKeyChecking=no "$ROCKNIX_USER@$known_rocknix_host" "[ -d \"$ROCKNIX_ROMS_PATH\" ] && echo exists" | grep -q "exists"; then
                echo "✅ Confirmed $ROCKNIX_ROMS_PATH exists on RockNix device"
            else
                echo "⚠️ Could not confirm $ROCKNIX_ROMS_PATH exists, but will use it anyway"
            fi
            
            # Skip the rest of the scanning since we found our target
            echo "RockNix host found via SSH. Skipping network scan."
            
            # Display results
            echo "✅ Found RockNix host: $known_rocknix_host"
            return 0
        else
            echo "❌ Direct SSH connection to $known_rocknix_host failed."
        fi
    else
    ...
    scan_ranges=(
        ...
        "130 135"
        # Additional targeted range around known RockNix host (132)
        "131 133"
        # Extended range to include 192.168.0.159
        "155 160"
    )
    ...
    # Try ping first on the known host
    echo "Checking if $known_rocknix_host is reachable via ping..."
    if ping -c 3 -W 3 "$known_rocknix_host" >/dev/null 2>&1; then
        echo "✅ $known_rocknix_host is reachable via ping"
        
        # Try multiple methods to check if SMB is available
        if nc -z -w 2 "$known_rocknix_host" 445 >/dev/null 2>&1; then
            echo "✅ $known_rocknix_host has SMB service (port 445 open)"
            DETECTED_SMB_HOSTS+=("$known_rocknix_host")
        elif nc -z -w 2 "$known_rocknix_host" 139 >/dev/null 2>&1; then
            echo "✅ $known_rocknix_host has SMB service (port 139 open)"
            DETECTED_SMB_HOSTS+=("$known_rocknix_host")
        elif nc -z -w 2 "$known_rocknix_host" 22 >/dev/null 2>&1; then
            echo "✅ $known_rocknix_host has SSH service (port 22 open)"
            echo "Assuming this is a RockNix device with SSH"
            DETECTED_SMB_HOSTS+=("$known_rocknix_host")
        else
            echo "⚠️ $known_rocknix_host is reachable but no SMB/SSH service detected"
            # Add it anyway since we know it's the RockNix host from configuration
            DETECTED_SMB_HOSTS+=("$known_rocknix_host")
        fi
    else
        echo "❌ $known_rocknix_host is not reachable via ping"
    fi
    
    # If we found the known host, skip the rest of the scan
    if [[ " ${DETECTED_SMB_HOSTS[@]} " =~ " $known_rocknix_host " ]]; then
        echo "Known RockNix host detected. Skipping further scanning."
        ROCKNIX_HOST="$known_rocknix_host"
    ...
    if [ $is_android -eq 1 ]; then
        ...
        for range in "${scan_ranges[@]}"; do
            ...
            for i in $(seq $start_ip $end_ip); do
                ...
                if [ "$target" = "$known_rocknix_host" ]; then
                    continue
                fi
                ...
                if ping -c 1 -W 1 "$target" >/dev/null 2>&1; then
                    ...
                    elif nc -z -w 1 "$target" 139 >/dev/null 2>&1; then
                        echo "SMB service found (port 139) ✅"
                        DETECTED_SMB_HOSTS+=("$target")
                    elif nc -z -w 1 "$target" 22 >/dev/null 2>&1; then
                        echo "SSH service found ✅"
                        DETECTED_SMB_HOSTS+=("$target")
                    else
                        echo "no SMB/SSH service ❌"
                    fi
                fi
            done
        done
                ...
                for i in $(seq $start_ip $end_ip); do
                    ...
                    if ping -c 1 -W 1 "$target" &>/dev/null; then
                        ...
                        if nc -z -w 1 "$target" 445 &>/dev/null; then
                            ...
                            DETECTED_SMB_HOSTS+=("$target")
                        elif nc -z -w 1 "$target" 22 &>/dev/null; then
    ...
    if [ ${#DETECTED_SMB_HOSTS[@]} -gt 0 ]; then
        ...
        # As a last resort, set the known host from configuration
        echo "Setting known RockNix host ($known_rocknix_host) from configuration."
        ROCKNIX_HOST="$known_rocknix_host"
        DETECTED_SMB_HOSTS+=("$known_rocknix_host")
    fi
    ...
    if [ ${#DETECTED_SMB_HOSTS[@]} -gt 0 ]; then
        ...
        for host in "${DETECTED_SMB_HOSTS[@]}"; do
            ...
            (
                ...
                sshpass -p "$ROCKNIX_PASS" ssh -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=/dev/null "$ROCKNIX_USER@$host" "mkdir -p $dest_dir" 2>/dev/null
                ...
                if sshpass -p "$ROCKNIX_PASS" scp -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=/dev/null "$rom_path" "$ROCKNIX_USER@$host:$dest_dir/"; then
                    ...
                    # Try alternative password if primary fails
                    if sshpass -p "$ROCKNIX_PASS_ALT" scp -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=/dev/null "$rom_path" "$ROCKNIX_USER@$host:$dest_dir/"; then
                        echo "✅ File transferred successfully to $host!"
                    else
...
show_menu() {
    ...
    echo "============================="
    echo "Select a platform or option:"
    echo "-----------------------------------------------------"
    echo "You can use ↑↓ arrows and Enter, or directly type a number (1-22)"
    echo "-----------------------------------------------------"
...
main_menu() {
    ...
    while true; do
        ...
        if [ "$choice" -ge 3 ] && [ "$choice" -le 20 ]; then
            case $choice in
                3) CURRENT_PLATFORM="nes";;
                4) CURRENT_PLATFORM="snes";;
                5) CURRENT_PLATFORM="gb";;
                6) CURRENT_PLATFORM="gba";;
                7) CURRENT_PLATFORM="gbc";;
                8) CURRENT_PLATFORM="sms";;
                9) CURRENT_PLATFORM="genesis";;
                10) CURRENT_PLATFORM="segacd";;
                11) CURRENT_PLATFORM="sega32x";;
                12) CURRENT_PLATFORM="saturn";;
                13) CURRENT_PLATFORM="gg";;
                14) CURRENT_PLATFORM="ngp";;
                15) CURRENT_PLATFORM="tg16";;
                16) CURRENT_PLATFORM="tgcd";;
                17) CURRENT_PLATFORM="ps1";;
                18) CURRENT_PLATFORM="ps2";;
                19) CURRENT_PLATFORM="n64";;
                20) CURRENT_PLATFORM="lynx";;
            esac
